...
class White_line_Detect:
    def __init__(self):
        ...
        rospy.Subscriber("/camera/color/image_raw/compressed", CompressedImage, self.img_CB)
    ...
    def detect_color(self, img):
        ...
        white_lower = np.array([0, 0, 200])                             #hsv 파일 통해 찾아낸 흰색의 범위 
        white_upper = np.array([179, 100, 255])

        # Threshold the HSV image to get only white colors
        white_mask = cv2.inRange(hsv, white_lower, white_upper)         #hsv 변환된 파일에 대해 흰색의 최소, 최대 
        cv2.imshow("img", img)
        cv2.imshow("img", hsv)
        cv2.imshow("white_mask", white_mask)
        white_color = cv2.bitwise_and(img, img, mask=white_mask)        # 원래 이미지와 마스크를 섞은 파일
